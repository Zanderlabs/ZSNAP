...
from . import eventmarkers
from . import OSCClient, OSCMessage
...
class BasicStimuli:
    ...
    class destroy_helper:
        """Small helper class to destroy multiple objects using destroy() call."""
    ...
    def __init__(self):
        ...
        # in write(), movie(), etc.
        self.extensive_markers = False  # whether to emit extensive string markers
        self._to_destroy = []
    ...
    def marker(self, markercode):
        """
        Emit a marker.
        The marker code can be a string or a number.
        Side note: strings will not work if a legacy marker sending protocol is enabled
        (such as DataRiver or the parallel port).
        """
        eventmarkers.send_marker(markercode)

    def write(self,
              text,  # the text to display
              duration=1.0,  # duration in seconds for which the text will be displayed
              # if this is a string, the stimulus will be displayed until the corresponding event is generated
              # if this is a list of [number,string], the stimulus will at least be displayed for <number> seconds,
              # but needs to confirmed with the respective event
              # if this is 0,
              # the write will be non-blocking and you have to .destroy() the return value of this function manually
              ...
              scale=0.07,
              # size of the text; either a single float (e.g. 0.07) or a 2-tuple of floats for non-uniform scaling
        ...
        obj = self._engine.direct.gui.OnscreenText.OnscreenText(text=text, pos=(pos[0], pos[1] - scale / 4), roll=roll,
                                                                scale=scale, fg=fg, bg=bg, shadow=shadow,
                                                                shadowOffset=shadowOffset, frame=frame, align=align,
                                                                wordwrap=wordwrap, drawOrder=drawOrder, font=font,
                                                                parent=parent, sort=sort)
        ...
        if block:
            ...
            if duration > 0:
                self._engine.base.taskMgr.doMethodLater(duration, self._destroy_object,
                                                        'ConvenienceFunctions, remove_text', extraArgs=[obj, 255])
            return obj
    ...
    def crosshair(self,
                  ...
                  # if this is a list of [number,string], the stimulus will at least be displayed for <number> seconds,
                  # but needs to confirmed with the respective event
                  # if this is 0, the write will be non-blocking and you have to .destroy()
                  # the return value of this function manually
        ...
        obj1 = self._engine.direct.gui.OnscreenImage.OnscreenImage(image='blank.tga', pos=(pos[0], 0, pos[1]),
                                                                   scale=(size, 1, width), color=color, parent=parent)
        self._to_destroy.append(obj1)
        obj1.setTransparency(self._engine.pandac.TransparencyAttrib.MAlpha)
        obj2 = self._engine.direct.gui.OnscreenImage.OnscreenImage(image='blank.tga', pos=(pos[0], 0, pos[1]),
                                                                   scale=(width, 1, size), color=color, parent=parent)
    ...
    def rectangle(self,
        ...
        t = rect[2]
        b = rect[3]
        obj = self._engine.direct.gui.OnscreenImage.OnscreenImage(image='blank.tga',
                                                                  pos=((l + r) / 2, depth, (b + t) / 2),
                                                                  scale=((r - l) / 2, 1, (b - t) / 2), color=color,
                                                                  parent=parent)
        self._to_destroy.append(obj)
        ...
        if block:
            ...
            if duration > 0:
                self._engine.base.taskMgr.doMethodLater(duration, self._destroy_object,
                                                        'ConvenienceFunctions, remove_rect', extraArgs=[obj, 251])
            return obj
    ...
    def frame(self,
        ...
        w = thickness[0]
        h = thickness[1]
        L = self._engine.direct.gui.OnscreenImage.OnscreenImage(image='blank.tga', pos=(l - w / 2, 0, (b + t) / 2),
                                                                scale=(w / 2, 1, w + (b - t) / 2), color=color,
                                                                parent=parent)
        L.setTransparency(self._engine.pandac.TransparencyAttrib.MAlpha)
        self._to_destroy.append(L)
        R = self._engine.direct.gui.OnscreenImage.OnscreenImage(image='blank.tga', pos=(r + w / 2, 0, (b + t) / 2),
                                                                scale=(w / 2, 1, w + (b - t) / 2), color=color,
                                                                parent=parent)
        R.setTransparency(self._engine.pandac.TransparencyAttrib.MAlpha)
        self._to_destroy.append(R)
        T = self._engine.direct.gui.OnscreenImage.OnscreenImage(image='blank.tga', pos=((l + r) / 2, 0, t - h / 2),
                                                                scale=(h + (r - l) / 2, 1, h / 2), color=color,
                                                                parent=parent)
        T.setTransparency(self._engine.pandac.TransparencyAttrib.MAlpha)
        self._to_destroy.append(T)
        B = self._engine.direct.gui.OnscreenImage.OnscreenImage(image='blank.tga', pos=((l + r) / 2, 0, b + h / 2),
                                                                scale=(h + (r - l) / 2, 1, h / 2), color=color,
                                                                parent=parent)
        ...
        if block:
            ...
            if duration > 0:
                self._engine.base.taskMgr.doMethodLater(duration, self._destroy_object,
                                                        'ConvenienceFunctions, remove_frame',
                                                        extraArgs=[[L, R, T, B], 243])
    ...
    def picture(self,
                ...
                # optional parameters:
                pos=None,
                # the (x,z) or (x,y,z) position of the image on the screen;
                # this may be a 3-tuple of floats; y should be zero
                hpr=None,
                # the (heading,pitch,roll) angles of the image; if this is a single number,
                # it will be taken as the roll angle
                scale=None,
                # the size of the image; this may be a single flot, a 3-tuple of floats, or a vector;
                # y should be 1, if a 3-tuple is given
        ...
        obj = self._engine.direct.gui.OnscreenImage.OnscreenImage(image=image, pos=pos, hpr=hpr, scale=scale,
        ...
        if block:
            ...
            if duration > 0:
                self._engine.base.taskMgr.doMethodLater(duration, self._destroy_object,
                                                        'ConvenienceFunctions, remove_picture', extraArgs=[obj, 249])
            return obj
    ...
    def sound(self,
              ...
              direction=0.0,
              # the balance; may be a number between -1 (hard left) and 1 (hard right), or an angle if surround=True
              playrate=1.0,  # the playrate of the sound (changes pitch and time)
              timeoffset=0.0,  # time offset into the file
              looping=False,
              # whether the sound should be looping; can be turned off by calling .stop()
              ...
              location='surround',
              # can be 'surround' (wall speakers), 'array' (sound bars), or 'headset' (traditional Windows sound)
        ...
        if self._oscclient is not None and (location == 'surround' or location == 'array'):
            ...
            msg = OSCMessage(destination)
            msg += [id, "vol", volume]
            oscclient.send(msg)
            msg = OSCMessage(destination)
            msg += [id, "clipname", filename]
            oscclient.send(msg)
            msg = OSCMessage(destination)
            msg += [id, "pos", direction * 180 / 3.1415, 0.0, distance]
            oscclient.send(msg)
            msg = OSCMessage(destination)
            msg += [id, "speed", playrate]
            oscclient.send(msg)

            if timeoffset > 0:
                print("Time offset is currently not supported in this interface")
            if loopcount is not None:
                print("Loop count is currently not supported in this interface")

            # determine the length of the file, so we can block for the appropriate time (and later reclaim sound ID's)
            obj = self._engine.loader.loadSfx(filename)
            length = obj.length()
            if loopcount is not None:
                length *= loopcount
            if looping:
            ...
            msg = OSCMessage(destination)
            msg += [id, "play", timeoffset, 0 if looping else loopcount]
            oscclient.send(msg)
            ...
            class Stopper:
                ...
                def destroy(self):
                    if self.enabled:
                        msg = OSCMessage(self.destination)
                        msg += [self.id, "stop"]
            ...
            if block:
                ...
                self._engine.base.taskMgr.doMethodLater(length, self._destroy_object,
                                                        'ConvenienceFunctions, end_sound',
                                                        extraArgs=[None, 247])
            ...
            if surround:
                if self.audio3d is None:
                    self.audio3d = self._engine.direct.showbase.Audio3DManager.Audio3DManager(
                        self._engine.base.sfxManagerList[0], None)
    ...
    def movie(self,
              ...
              # optional parameters:
              pos=None,
              # the (x,z) or (x,y,z) position of the video on the screen;
              # this may be a 3-tuple of floats; y should be zero
              hpr=None,
              # the (heading,pitch,roll) angles of the video;
              # if this is a single number, it will be taken as the roll angle
              scale=None,
              # scaling of the video area;
              # this may be a single float, an (x,z) value, or an (x,y,z) value; note that one of
              # the scale axes is shrunken according to the value of the "aspect" property
              # (which is auto-deduced unless manually overridden)
              ...
              looping=False,
              # whether the video should be looping;
              # can be turned off by calling .stop() on the return value of this function
              loopcount=None,  # optionally the number of repeats if looping
              aspect=None,
              # aspect ratio of the video (auto-deduced if None); example: for 16:9 this would be 16.0/9.0
              # the smaller axis of the video area is automatically shrunken based on this number
              pixelscale=False,
              # if this is True,
              # aspect will be ignored and the image will be scaled according to the pixels in the image
              ...
              contentscale=None,
              # [u,v] scale of the movie content within the viewport/texture
              # -- if None this is automatically deduced based on the
              # texture size and the size of the video content within the texture
              # (note that it can be tricky to get this right due
              # to possible padding introduced when the movie is loaded)
              bordercolor=(0, 0, 0, 0),
              # the border color of the movie texture (only visible when the contentoffset and contentscale are used
              ):
        """Play a movie.
        Note: Sound for movies only works with OpenAL (rather than FMOD)
        -- see documentation at http://www.panda3d.org/manual/index.php/Sound on how to select it."""
        if self.extensive_markers:
            self.marker("BasicStimuli::movie(filename=%s)" % filename)

        # load the soundtrack if there is one
        try:
        ...
        img = self._engine.direct.gui.OnscreenImage.OnscreenImage(image=tex, pos=pos, hpr=hpr, scale=scale, color=color,
                                                                  parent=parent)
        ...
        if block:
            ...
            self._engine.base.taskMgr.doMethodLater(length, self._destroy_object, 'ConvenienceFunctions, remove_movie',
                                                    extraArgs=[[img, tex, snd], 245])
            return playable
    ...
    def log_setup_parameters(self, extra_msg=''):
        ...
        """
        self.marker('Experiment Control/Setup/Parameters/%s:"%s"%s' % (
            self.__class__, str(self.__dict__).replace('"', '\\"'), extra_msg))

    def set_engine(self,
                   base,
                   # the global base object
                   # -- contains core global runtime variables of Panda3d (taskMgr, eventMgr, jobMgr, render, ...)
                   direct,  # the Python layer of Panda3d
                   # -- contains packages task, showbase, actor, and so on
                   pandac # the C++ layer of Panda3d (this is actually the package pandac.PandaModules)
                   # -- contains modules like NodePath, Point3, and so on
                   ):
        """
        This function allows to retarget the engine used for stimulus presentation,
        for example to present stimuli on a different computer.
        Note that this incurs significant latency,
        thus it can not be used for latency-sensitive stimulus presentation.
        """
