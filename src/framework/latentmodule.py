...
from . import BasicStimuli
from . import TickModule
from . import shared_lock


class LatentModule(TickModule, BasicStimuli):
    """
    ...
    def __init__(self,
                 ...
                 make_up_for_lost_time=True,
                 # whether time lost during a sleep(), e.g., due to jitter, will be compensated over successive sleep's
                 max_compensated_time=0.5,
                 # the maximum amount of lost time prior to any sleep() call that will be compensated for
                 # the maximum frame time during normal operation that is not considered a hickup, in seconds
                 max_inter_frame_interval=0.5):
        """
        Construct a new LatentModule; a default tick function (called at every frame) may be specified here.
        This is the place to assign default values and arguments to all variables. Ideally you should defer
        any processing of these variables (such as loading text files) to the beginning of the run() function,
        since the default values may be overridden by the experimenter or config files
        ...
        # condition variable that signals that the sleep period is over
        self._resumecond = threading.Condition(shared_lock)
        self._cancelled = False
        # signals whether cancel() has been invoked (i.e. that run() shall terminate at the next opportunity)

        now = time.time()
        # the point in time when the currently running time-consumption function should resume (if any)
        self._resumeat = now
        ...
        self._frametime = 1 / 60.0  # initial guess of time between frames (updated every frame)
        # whether time lost during a sleep(), e.g., due to jitter, will be compensated over successive sleep's
        self._make_up_for_lost_time = make_up_for_lost_time
        # the maximum amount of lost time prior to any sleep() call that will be compensated for
        self._max_compensated_time = max_compensated_time
        # the maximum reasonable frame time during normal operation, in seconds
        self._max_inter_frame_interval = max_inter_frame_interval

        self._cur_tick = None  # the tick function that is currently in place
        # the tick function that is running whenever no current tick function is specified
    ...
    def _run_wrap(self):
        ...
        try:
            ...
            # the ModuleCancelled exception is used to forcibly exit the run() function
            pass
